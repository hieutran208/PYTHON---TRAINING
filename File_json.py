...
with open('exp.json',mode='w',encoding='utf-8') as file:
    ...
    json.dumps(dict_data, indent=4, ensure_ascii=False) # Còn gọi là Pretty print trong Python
...
with open('exp.json',mode='r',encoding='utf-8') as file: # encoding="utf-8" để tránh lỗi Unicode
    ...
    print(data)
    print(type(data)) 
    print(data["skills"]) # có thể dùng data.get("skills") để truy cập dữ liệu
    print(data["address"]["city"]) # Đọc JSON lồng nhau (còn gọi là parse nested JSON)
